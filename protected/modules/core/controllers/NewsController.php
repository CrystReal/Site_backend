<?php

/**
 * User: David
 * Date: 1/8/14 10:26 PM
 */
class NewsController extends MainController
{
    public function actionIndex()
    {
        $base = News::model()->findAll();
        $this->render('index', array('base_model' => $base));
    }

    public function actionCreate()
    {
        $this->actionUpdate(true);
    }

    public function actionUpdate($new = false)
    {
        if ($new)
            $base = new News();
        else
            $base = News::model()->findByPk($_GET['id']);
        if (Yii::app()->request->isPostRequest) {
            $base->attributes = $_POST['News'];
            $base->image = CUploadedFile::getInstanceByName("image");
            if (!$base->hasErrors() && $base->save()) {
                $this->redirect($this->createUrl("index"));
            }
        }
        $this->render('add_edit', array('model' => $base));
    }

    public function actionDelete()
    {
        if (News::model()->deleteByPk($_GET['id'])) {
            Yii::app()->user->setFlash('success', "Удалено!");
            $this->redirect($this->createUrl("index"));
        } else {
            Yii::app()->user->setFlash('error', "Ошибка!");
            $this->redirect($this->createUrl("index"));
        }
    }
}